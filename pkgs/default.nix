flakes: final: prev: with final; {
  ...
  crowdsec =
    (
      if lib.versionOlder prev.crowdsec.version "1.6.0" then
        flakes.nixpkgs.legacyPackages.${system}.crowdsec
      else
        prev.crowdsec
    ).overrideAttrs
      (orig: {
        ldflags = builtins.map (lib.replaceStrings [ "refs/tags/" ] [ "" ]) orig.ldflags;
      });
  ...
  opensshWithKerberos =
    if lib.hasAttr "opensshWithKerberos" prev then prev.opensshWithKerberos else prev.openssh;
